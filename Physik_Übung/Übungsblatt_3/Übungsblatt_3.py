...
x_plot = np.linspace(100, 1000, 100000)
plt.plot(x_plot, (np.sqrt(x_plot)-np.sqrt(x_plot-100))/(np.sqrt(x_plot-100)+np.sqrt(x_plot)) ,'g-', label="$r$")
plt.plot(x_plot, (2*np.sqrt(x_plot))/(np.sqrt(x_plot-100)+np.sqrt(x_plot)) ,'r-', label="$t$")
plt.legend(loc="best")
plt.xlabel(r'Energie ')
plt.ylabel(r'$r/t$')
plt.grid()
