...
\begin{align}
    ...
    n_{Cu} &= \text{\input{n_Cu_IB.tex}} \label{eq:18b}
\end{align}
\end{subequations}
...
\begin{align}
    R_{H,Cu} = \text{\input{AH_Cu_IB.tex}} \label{eq:19}
\end{align}
...
\begin{align}
        \overline{\tau}_{Cu} &= 2\frac{m_0}{e_0^2}\frac{1}{n\varrho} = \text{\input{tau_Cu_IB.tex}}
    \intertext{
    ...
    $n_{Cu}$ \eqref{eq:18b} eingesetzt, woraus folgt:
        ...
        \overline{v}_{d,Cu} &= \frac{-j}{n e_0} = \text{\input{v_d_Cu_IB.tex}} \label{eq:21}
    \intertext{
    ...
    nach $\mu$ umgestellt und die Werte für $\overline{v}_{d,Cu}$ \eqref{eq:21} und $E$ \eqref{eq:22} eingesetzt:
        ...
        \mu_{Cu} &= \frac{\overline{v}_{d,Cu}}{E} = \text{\input{mu_Cu_IB.tex}} \label{eq:23}
    \intertext{
    ...
    \intertext{
        ...
        \abs{v}_{Cu} &\approx \sqrt{\frac{2 E_F}{m_0}} = \text{\input{v_Cu_IB.tex}} \label{eq:25}
    \intertext{
        Mit der absoluten Geschwindigkeit lässt sich $\overline{l}$ approximieren als:   
    }
        \overline{l}_{Cu} &\approx \overline{\tau}\sqrt{\frac{2 E_F}{m_0}} = \text{\input{l_Cu_IB.tex}} \label{eq:26}
\end{align}

\subsubsection{Konstantes B-Feld} \label{sec:5.3.2}

\flushleft{Die\;}\justifying Werte für ein konstantes B-Feld werden analog zu Abschnitt \ref{sec:5.3.1} berechnet. Die Ergebnisse lauten wie folgt:
...
\subsection{Konstanter Querstrom} \label{sec:5.4.1}

\flushleft{Die\;}\justifying Werte für einen konstanten Querstrom werden analog zu Abschnitt \ref{sec:5.3} berechnet. Die Ergebnisse lauten wie folgt:
...
\centering
    ...
    \toprule
        ...
        \bottomrule
    \end{tabular}
\caption{Werte für Kupfer mit konstantem Querstrom}
\label{tab:6}
\end{table}

\subsection{Konstantes B-Feld} \label{sec:5.4.2}

\flushleft{Die\;}\justifying Werte für ein konstantes B-Feld werden analog zu Abschnitt \ref{sec:5.3} berechnet. Die Ergebnisse lauten wie folgt:
...
\subsection{Konstanter Querstrom} \label{sec:5.5.1}
...
\subsection{Konstantes B-Feld} \label{sec:5.5.2}
