...
\flushleft{Es\;}\justifying werden die drei Materialien Kupfer, Silber und Zink verwendet. Zuerst wird der spezifische Widerstand der einzelnen
...
\begin{align}
    \rho_{Cu} &= \text{\input{rho_Cu.tex}} \label{eq:15a}\\
    \rho_{Ag} &= \text{\input{rho_Ag.tex}} \label{eq:15b}\\
    \rho_{Zn} &= \text{\input{rho_Zn.tex}} \label{eq:15c}
\end{align}
\end{subequations}
...
\flushleft{Für\;}\justifying die Ladungsträger pro Volumen wird die Formel \eqref{eq:1} nach $n$ umgestellt. 
...
\flushleft{Die\;}\justifying mittlere Driftgeschwindigkeit
