...
\begin{align}
    I(\varphi) \propto  B(\varphi)^2 = A_0^2 b^2 \left\{\frac{\lambda}{\pi b \sin \varphi} \right\}^2 \sin ^2 \left(\frac{\pi b \sin \varphi}{\lambda} \right) \text{\cite{V406}} \label{eq:4}
\end{align}
gemessen.\\
...
\begin{figure}[H]
    ...
    \label{fig:3}
\end{figure}
\flushleft{Zuerst\;}\justifying wird der Dunkelstrom des Photoelement gemessen, indem die Stromstärke
bei verdeckter Öffnung bestimmt wird.
\flushleft{Dann\,}\justifying wird der Laser in Richtung der Blende und parallel zur Messschiene
...
Bei der Durchführung des Experiments wird ein Laser der Wellenlänge $\lambda = \SI{532}{\nano\meter} $ verwendet.
...
\begin{subequations}
\begin{align}
    I(\varphi) &= A^2\cdot b^2 \,\text{sinc} \left(\frac{b \sin{\varphi}}{\SI{532}{\nano\meter}} \right) \label{eq:6a}\\
    A &= \input{A.tex} \label{eq:6b}\\
    b &= \input{b.tex} \label{eq:6c}
\end{align}
\end{subequations}
...
\end{align}
Eine Erklärung für den Fehler könnte unter anderem
...
sein, wodurch das Beugungsbild verändert werden würde.\\ 
\newpage
\printbibliography
