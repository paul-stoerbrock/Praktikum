...
\begin{equation}
T^2 = I_k \cdot \frac{4\pi^2}{D} + \frac{m \cdot 4\pi^2}{D} \cdot a^2\label{placeholder}
\end{equation} 
...
\flushleft{Für\;}\justifying die Federkonstante wurde die Formel \eqref{eq:7} mit der Formel \eqref{eq:9} gleichgesetzt. Formel \eqref{eq:7} kann durch $M = \vec{F} \cdot \vec{r}$
...
\begin{equation}
D = \frac{F\cdot r}{\varphi}\label{eq:12}
\end{equation}
...
\begin{equation}
\overline{D} = \text{\input{D_mean.tex}} \label{eq:13} % Federkonstante der Spiralfeder
\end{equation}
