...
\intertext{Den Mittelwert wird berechnet mit}
     \overline{x} &= \frac{1}{N}\sum_{i=1}^{N} x_i \label{eq:7}.
\intertext{Der Fehler des Mittelwerts wird berechnet mit}
        \Delta\overline{x} &= \frac{1}{\sqrt{N}} \sqrt{\frac{1}{1-N} \sum_{i=1}^{N} (x_i - \overline{x})^2} \label{eq:8},
\intertext{Die Gaußsche Fehlerfortpflanzung wird mit der folgenden Formel berechnet} \text{und}
        \Delta f &= \sqrt{\sum_{i=1}^{N} \left( \frac{\delta f}{\delta x_i} \right)^2 \cdot (\Delta x_i)^2} \label{eq:9}
        \intertext{verwendet.}
