...
\flushleft{Die\;}\justifying folgenden Maße werden mit einer Schieblehre bestimmt,
...
\end{align*}
Das Gewicht der einzelnen Puppenteile wurde dabei über das Volumen und die recherchierte Holzdichte 
berechnet:
\begin{align*}
    m = \rho \cdot V
\end{align*}
...
\flushleft{Mithilfe\;}\justifying des Phyton Befehls linregress() \cite{numpy} wird die Geradengleichung
des folgenden Graphen \ref{fig:1} bestimmen. 
\flushleft{Dabei\;}\justifying habe die Steigung $m$ und der Schnittpunkt der y-Achse $b$ des Graphen \ref{fig:1} folgende Werte:
...
\frac{\abs{\overline{I_{Zy, exp.}} - I_{Zy, lit.}}}{I_{Zy, lit.}} &= \input{RF_I_Zylinder.tex}\label{eq:19c}
\end{align}
\end{subequations}
...
\frac{\abs{\overline{I_{Ku, exp.}} - I_{Ku, lit.}}}{I_{Ku, lit.}} &= \input{RF_I_Kugel.tex} \label{eq:20c}
\end{align}
\end{subequations}
...
\label{fig:2}
    ...
    \end{subfigure}
\end{figure}
\flushleft{Die\;}\justifying Massen der einzelnen Zylinder werden mithilfe der Formel $m= \rho \cdot V$ bestimmt und lauten wie folgt:
...
\frac{\abs{\overline{I_{P1, exp.}} - I_{P1, lit.}}}{I_{P1, lit.}} &= \input{RF_I_Puppe_an.tex}\label{eq:24d}
\end{align}
\end{subequations}
...
\frac{\abs{\overline{I_{P2, exp.}} - I_{P2, lit.}}}{I_{P2, lit.}} &= \input{RF_I_Puppe_aus.tex}\label{eq:25c}
\end{align}
\end{subequations}
\newpage
...
\flushleft{Werden\;}\justifying nun nur die Literaturwerte des Zylinders und der Kugel betrachtet, lässt sich ein gutes Verhältnis erkennen.
...
das der Kugel \eqref{eq:20a} und das des Zylinders \eqref{eq:19a}, sind negativ, was pyhsikalisch unrealistisch erscheint, da 
