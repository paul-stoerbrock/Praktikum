...
\flushleft{Anhand\;}\justifying des Graphen \ref{fig:4} ohne Störung und dessen Parameter $A$ \eqref{eq:4} lässt sich die angelegte Spannung 
\begin{align}
    A = \text{\input{A_os.tex}}
\end{align}
bestimmen. Wird diese Spannung mit der Amplitude $U = \SI{3.25}{\volt}$ aus Abbildung \ref{fig:7a} verglichen, lässt sich ein Verstärkungsfakor
von $\sfrac{A}{U} = \text{\input{faktor_os.tex}}$ berechnen. Der Graph \ref{fig:4} gibt einen erwarteten Amplitudenverlauf des Signals 
$U_{sig}$ ohne Störung wieder. 
...
\begin{align}
    C = \text{\input{A_ms.tex}}
\end{align}
bestimmen. Um den Verstärkungsfakor zu berechnen, wird hier ebenfalls die Amplitude $U = \SI{3.25}{\volt}$ aus Abbildung \ref{fig:7a} verwendet.
Daraus folgt der Verstärkungsfakor des Signals von $\sfrac{C}{U} = \text{\input{faktor_ms.tex}}$ mit Störung.
...
\ref{tab:3} sollte mit einem Faktor von $\sfrac{1}{r^2}$ abfallen. Dies ist jedoch nicht der Fall. Die nicht-lineare Regression aus \ref{fig:6} 
zeigt das eigentliche Verhalten der Intensität. Die Messwerte verhalten sich jedoch anders.
% Literatur %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
