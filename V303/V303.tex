...
\begin{align}
    ...
    A&=\text{\input{I_A.tex} \quad \text{und}}\\
    B&=\text{\input{I_B.tex}}
\end{align}
\end{subequations}
...
\flushleft{Die\;}\justifying folgenden Abbildungen zeigen die Parameter der Signale $U_{sig}$ (CH2) und $U_{ref}$ (CH1). Hier ist $U_{ref}$ (CH1)
das variable Signal.
...
\flushleft{Anhand\;}\justifying des Graphen \ref{fig:5} und dessen Parameter $A$ \eqref{eq:4} lässt sich die angelegte Spannung 
\begin{align}
    A = \text{\input{A_os.tex}}
\end{align}
bestimmen. Wird diese Spannung mit der Amplitude aus Abbildung \ref{fig:7b} von $U = \SI{3.25}{\volt}$ verglichen, lässt sich ein Verstärkungsfakor
von $\sfrac{U_0}{U} = [missing\;value]$ berechnen. Der Graph $[reference]$ gibt einen erwarteten Amplitudenverlauf des Signals $U_{sig}$ ohne Störung
wieder. 
