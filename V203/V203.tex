...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./build/test.pdf}
    \caption{Plot für das Druck-Temperatur Verhältnis }
    \label{fig:5}
\end{figure}
...
\begin{subfigure}{0.495\linewidth}
    ...
    \label{fig:6b}
\end{subfigure}
\caption{Plots für die Temperaturabhängigkeit\label{fig:6}}
\end{figure}
...
\begin{equation}
    \frac{\abs{L_{Exp.} - L_{Lit.}}}{L_{Lit.}} = \text{\input{Fehler_Lk1b.tex}} \label{eq:15}
\end{equation}
ergibt. Der relative Fehler \eqref{eq:15} ist gering, was einen systematischen Fehler vermuten ließe. Die Ungenauigkeit beim Ablesen des Thermometers
...
Es wird erst in den darauf folgenden Graphen \ref{fig:6a} und \ref{fig:6b} deutlich, dass $V_{D,+}$ ein 
besseres Ergebnis als Dampfvolumen liefert. Die Messwerte im Graphen \ref{fig:6a} verlaufen deutlicher entlang der Ausgleichsgeraden, wohingegen die Werte im Graphen
\ref{fig:6b} einer Kurve ähneln. Zudem erscheint der Graph \ref{fig:6a} von $V_{D,+}$ physikalisch sinnvoller, da mit steigender
