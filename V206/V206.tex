...
\flushleft{Das\;}\justifying Experiment wird wie in Abbildung \ref{fig:1} aufgebaut:
...
\flushleft{Die\;}\justifying Temperatur der einzelnen Reservoire werden mithilfe der Messwerte im Anhang \ref{sec:7} erstellt. Die relevanten
Parameter $m$ und $b$, wovon $m$ mit der Formel \eqref{eq:10e} berechnet wird, lauten wie folgt:
\begin{subequations}
\begin{align}
    m_{T1} &= \text{\input{parT1_m.tex}} \qquad \text{und} \qquad &b_{T1} = \text{\input{parT1_b.tex}} \label{eq:11a}\\
    m_{T2} &= \text{\input{parT2_m.tex}} \qquad \text{und} \qquad &b_{T2} = \text{\input{parT2_b.tex}} \label{eq:11b}
\end{align}
\end{subequations}

\begin{figure}[H]
\centering
\caption{Temperaturverlauf der Reservoire}
\label{fig:2}
\begin{subfigure}{0.495\linewidth}
    \includegraphics[width=\textwidth]{build/plotT1.pdf}
    \caption{Temperaturverlauf Reservoir 1}
    \label{fig:2a}
\end{subfigure}
\begin{subfigure}{0.495\linewidth}
    \includegraphics[width=\textwidth]{build/plotT2.pdf}
    \caption{Temperaturverlauf Reservoir 2}
    \label{fig:2b}
\end{subfigure}
\end{figure}

\flushleft{Um\;}\justifying die zwei Differentialquotienten $\sfrac{d_{T1}}{dt}$ und $\sfrac{d_{T2}}{dt}$ zu berechnen werden die Formel
\begin{align}
