...
Damit wird Die Güteziffer $\nu$ wie folgt berechnet \cite{V206}:
\begin{align}
...
Die letzte relevante Größe ist die mechanische Kompressorleistung $N_{mech} $, welche die
...
ein Drosselventil, Dichlordifluormethan, ein Reiniger, ein Wattmeter, eine Stoppuhr, ein Messkolben, und $\SI{6}{\liter}$ Wasser.}
...
die Temperaturen der beiden Reservoire 1 und 2, die Drücke $p_a$ und $p_b$, und die erbrachte Leistung $P$ des Kompressors abgelesen. Es werden
...
\flushleft{Der\;}\justifying Temperaturverlauf der einzelnen Reservoire werden mithilfe der Messwerte aus Tabelle \ref{tab:1} erstellt. Die für
...
\flushleft{Um\;}\justifying die zwei Differentialquotienten $\sfrac{\Delta T_{1}}{\Delta t}$ und $\sfrac{\Delta T_{2}}{\Delta t}$ zu berechnen, wird die Formel
\begin{align}
    \frac{\Delta T(t)}{\Delta t} = 2At + B \label{eq:12}
\end{align}
...
\end{align}
Um mit der Fehlerfortpflanzung \eqref{eq:13} auf die Differentialquotienten $\sfrac{\Delta T_1}{\Delta t}$ und $\sfrac{\Delta T_2}{\Delta t}$ 
zu schließen werden die Temperaturen $t = \SI{26.6}{\celsius}$, $\SI{39.9}{\celsius}$, $\SI{47.4}{\celsius}$ und $\SI{49.6}{\celsius}$ für $T_1$ 
verwendet. Für $T_2$ werden die Temperaturen $t = \SI{17.4}{\celsius}$, $\SI{7.8}{\celsius}$, $\SI{1.4}{\celsius}$ und $\SI{-0.4}{\celsius}$ benutzt.
...
den vier Werten für $\sfrac{\Delta Q_2}{\Delta t} $ aus Tabelle \ref{tab:3} bestimmt.
...
\flushleft{Für\;}\justifying die mechanische Leistung $N_{mech}$ des Kompressors wird die Formel \eqref{eq:5} und die für Dichlordifluormethan spezifischen Werte 
...
\begin{align}
    N_{mech1} &= \text{\input{N0.tex}} \qquad \text{mit} \qquad t = \SI{29}{\minute} \label{eq:18a}\\
    N_{mech2} &= \text{\input{N1.tex}} \qquad \text{mit} \qquad t = \SI{25}{\minute} \label{eq:18b}\\
    N_{mech3} &= \text{\input{N2.tex}} \qquad \text{mit} \qquad t = \SI{15}{\minute} \label{eq:18c}\\
    N_{mech4} &= \text{\input{N3.tex}} \qquad \text{mit} \qquad t = \SI{4}{\minute} \label{eq:18d}
\end{align}
\end{subequations}
...
\flushleft{Die\;}\justifying Differenzen \eqref{eq:19} vergrößern sich, was den physikalischen Erwartungen gleicht. Da es sich hier um 
...
nicht deckungsgleich. Der neunte Wert im Graphen \ref{fig:3a} weicht ein wenig von der nicht-linearen Regressionslinie ab. Dies ließe sich
dadurch erklären, dass beim notieren der Messwerte nicht rechtzeitig der Wert abgelesen wurde. 

\flushleft{Der\;}\justifying Graph der Verdampfungswärme \ref{fig:4} erscheint ebenso physikalisch sinnvoll, da die Verdampfungswärme von 
...
der Ausgleichsgeraden. Dieser könnte sich durch eine Messungenauigkeit beim Ablesen während des Versuchs erklären lassen.

\flushleft{Bei\;}\justifying der mechanischen Leistung $N_{mech}$ \eqref{eq:18} wird deutlich, dass die Arbeit bei vorranschreitender Zeit abnimmt. 
...
\flushleft{Die\;}\justifying Größenordnung der realen Güteziffern $\nu_{real}$ aus Tabelle \ref{tab:3} ist deutlich kleiner als die der idealen. 
...
auf Werten aufbauen, welche mithilfe von Näherungen berechnet wurden, wie zum Beispiel $\sfrac{\Delta Q_1}{\Delta t}\; \text{und}\; \sfrac{1}{N}$, verliert das 
Endergebnis an Genauigkeit. 
